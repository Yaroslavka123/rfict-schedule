...
**Backend — единственный источник истины.** Фронт больше не читает GitHub raw или `public/schedule/*.json` — если backend недоступен, приложение покажет ошибку. Поэтому требования ниже обязательны.
  ...
  "groups": [
    {
      "id": "ИКБО-01-23",
      "name": "ИКБО-01-23",
      "specialty": "Радиофизика и цифровые технологии",
      "department": "РФиИТ"
    }
  ],
  "lessons": [
...
### 2.2. Как проверить installable trigger

1. Откройте таблицу и обновите страницу, чтобы появилось меню `Расписание`.
2. Нажмите `Расписание → Автосохранение на сайт: выкл/вкл` и подтвердите включение.
3. Откройте Apps Script → `Triggers` (иконка часов). Должен быть ровно один trigger:
...
Фронт **не кэширует** план в `localStorage` и не имеет других источников данных — если backend отвечает ошибкой или недоступен, пользователь видит экран ошибки. Backend обязан всегда возвращать расписание и план.

## Обновления с минимальной задержкой

Это обязательное требование для удобной работы: когда Apps Script parser отправил новое расписание в backend, frontend должен узнать об этом почти сразу. Без SSE/WebSocket или version-endpoint фронт увидит новые данные только по кнопке «Обновить».
...
data: {"type":"schedule_updated","course":3,"week_number":14,"updated_at":"2026-05-17T19:00:00Z","version":"2026-05-17T19:00:00Z","source":"apps_script_autosave"}
```
