...
- матрица занятости кабинетов;
- список преподавателей и их занятий;
- базовая аналитика и план-факт поверх данных расписания;
- fallback на статические JSON, если backend API временно недоступен.

Backend нужен как источник расписания, справочников, плановых показателей и событий обновления. Всё, что можно безопасно посчитать из уже загруженного расписания, считается на frontend.

Для минимальной задержки обновлений frontend должен работать через backend API, а не только через GitHub JSON fallback.
