...
│   ├── GeneratorDialog.html # Диалог генератора
│   ├── SetupDialog.html   # Мастер первоначальной настройки
│   ├── Sidebar.html       # UI формы ввода занятий (sidebar в Google Sheets)
│   ├── appsscript.json    # Манифест Apps Script (OAuth scopes, timezone)
│   ├── INSTALLATION.md    # Простая установка через копию эталонной таблицы
...
2. Создай копию и обнови страницу.
3. Открой **«Расписание → Первоначальная настройка»**.
4. При необходимости введи код публикации, выданный администратором.
5. Нажми **«Настроить таблицу»**, затем создай недели через генератор.

Подробно: [`apps_script/labs_form/INSTALLATION.md`](apps_script/labs_form/INSTALLATION.md)
...
Основной способ — копия эталонной Google-таблицы. Привязанный Apps Script, HTML-диалоги и встроенный шаблон генератора копируются вместе с ней. Мастер первоначальной настройки создаёт installable-триггер и проверяет конфигурацию.

Ручная установка через редактор Apps Script оставлена только для разработчика.
...
Триггер `onSheetEdit` создаётся мастером первоначальной настройки.
...
Для добавления нового курса: создай копию эталонной таблицы, запусти первоначальную настройку и сгенерируй листы нужного курса.
...
- Перед включением проверь, что генератор записал правильный номер курса в `A2` листов недели.
- При включении/выключении — confirm-диалог.

**Ручной экспорт:** меню **«Расписание → 💾 Сохранить расписание сейчас»** — мгновенный, без debounce. Требует включённое автосохранение; перед отправкой показывает confirm-диалог.
...
| Начало 1-й пары | `HH:MM` | `08:30` |
| Длительность пары | Минуты | 90 |
| Перерыв между парами | Минуты | 10 |
| Количество пар в день | | 8 |
| Длительность большого перерыва | Всегда ставится после 5-й пары | 40 |

Дата на листах всегда записывается без года в формате `DD.MM`.

### Алгоритм расчёта дат

```
семестр 1 (осенний):  старт = первый понедельник не раньше 1 сентября {year}
семестр 2 (весенний): старт = первый понедельник не раньше 8 февраля {year+1}
```

Это исключает создание лишней недели до начала семестра. Например, весенний
семестр 2026 начинается с листа `09.02-14.02 (1-я неделя)`.
...
3. **Колонка A, ячейки дней** — над оригинальным текстом дня дописывается дата:
   ...
   мельче и курсивом (через RichText).
