...
идентичные пустые листы — это фундамент для будущего генератора семестра.
...
Шаблон хранится в скрытом листе `_TEMPLATE_` как JSON, разбитый на чанки по
45 000 символов (чтобы избежать лимита Apps Script на размер ячейки в 50 000).
Этот лист не экспортируется на бэк — фильтр `weekSheets` в `getSheetMeta_`
матчит только листы по регэкспу `неделя\)?\s*$/`.
