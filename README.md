...
При **любом изменении** в таблице (ручном или через форму) запускается отложенный экспорт — **но только если включён Push-режим** (по умолчанию выключен).
...
**Push-режим (тумблер в меню):**

- Меню **«Расписание → 🚀 Push в GitHub: ВКЛ/ВЫКЛ»** — глобальный тумблер автоэкспорта. По умолчанию **ВЫКЛ** (тестовый режим).
- Хранится в `DocumentProperties` (ключ `PUSH_ENABLED`).
- Чтобы включить — нужно сначала указать **номер курса** через **«🎓 Курс: …»** (ключ `COURSE_OVERRIDE`). Это защита от записи в чужую папку из новой/чужой таблицы.
- При включении/выключении — confirm-диалог.

**Тестовый прогон:** меню **«Расписание → 🧪 Тестовый прогон (parse без push)»** — парсит активный лист недели и показывает summary (кол-во групп, lesson'ов, разбивка по типам) без отправки в GitHub. Работает независимо от Push-тумблера.

**Ручной экспорт:** меню **«Расписание → ⚡ Обновить расписание сейчас»** — мгновенный, без debounce. Требует Push: ВКЛ и указанный курс; перед отправкой показывает confirm-диалог.
